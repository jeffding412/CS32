...
int main()
{
    ...
    // destruction time.
    {
        ...
        for (int x = 0; x < auxSensors.size(); x++) {
            sensorPointers.push_back(&auxSensors[x]);
        }
        
        sort(sensorPointers.begin(), sensorPointers.end(), compareSensorPtr);
        
        for (int x = 0; x < sensorPointers.size(); x++) {
            sensors[x] = *sensorPointers[x];
        }
